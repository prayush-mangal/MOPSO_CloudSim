...
public abstract class Particle {
	...
	public void allocate(int dimention) {
		...
		this.bestFitness=new double[dimention];
		this.fitness=new double[dimention];
		for( int i = 0; i < position.length; i++ ) {
			bestPosition[i] = Double.NaN;
			fitness[i]=Double.NaN;
			bestFitness[i]=Double.NaN;
		}
	}
	...
	public void copyPosition(double positionCopy[],int i) {
		
			positionCopy[i] = position[i];
	}

	/** Copy position[] to bestPosition[] */
	public void copyPosition2Best(int i) {
	...
	public void setFitness(double[] fitness, boolean maximize) {
		...
		if( (maximize && (fitness[i] > bestFitness[i])) // Maximize and bigger? => store data
				|| (!maximize && (fitness[i] < bestFitness[i])) // Minimize and smaller? => store data too
				|| Double.isNaN(bestFitness[i]) ) {
			copyPosition2Best(i);
	...
	/**
	 * å�¯åŠ¨å�˜å¼‚ç­–ç•¥ éœ€è¦�åŠ å…¥å�˜å¼‚ç­–ç•¥æ—¶é‡�å†™è¯¥æ–¹æ³•
	 */
